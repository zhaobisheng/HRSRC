...
class ListController extends Controller {
    ...
    public function index($key = "") {
        ...
        $count = $model->where('post.type>3', $where)->count(); // 查询满足要求的总记录数
        $Page = new \Extend\Page($count, 5); // 实例化分页类 传入总记录数和每页显示的记录数(15)
        $show = $Page->show(); // 分页显示输出

        $post = $model->limit($Page->firstRow . ',' . $Page->listRows)->where('post.type>3', $where)->order('post.id DESC')->select();   //where('user_id='.$id)->

        $ncount = $model->where('post.type<=3', $where)->count(); // 查询满足要求的总记录数
        $nPage = new \Extend\Page($ncount, 5); // 实例化分页类 传入总记录数和每页显示的记录数(15)
        $nshow = $nPage->show(); // 分页显示输出

        $new = $model->limit(10)->where('post.type<=3', $where)->order('post.id DESC')->select();
    ...
    public function newlist($key = "") {
        ...
        $new = $model->limit($nPage->firstRow . ',' . $nPage->listRows)->where('post.type<=3', $where)->order('post.id DESC')->select();
        $this->assign('newBug', $new);
    ...
    public function view() {
        ...
        $post = $model->where('type>3 ANd  id=' . $rid)->find();  //修复越权漏洞
        if (empty($post)) {
